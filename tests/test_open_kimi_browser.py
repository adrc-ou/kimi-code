...
class BrowserTests(unittest.TestCase):
    def test_banner_order_local_first_and_ignores_unrelated_links(self):
        ...
        self.assertEqual(browser.advertised_urls(logs), [LOCAL, NETWORK, "http://172.24.0.3:5494/#token=fixture"])
    ...
    def test_local_unreachable_tries_network_without_sending_token(self):
        ...
        with patch.object(browser.urllib.request, "build_opener") as build:
            build.return_value.open.side_effect = [urllib.error.URLError("unreachable"), response, response]
    ...
    def test_all_unreachable_never_opens_browser(self):
        with patch.object(browser.subprocess, "run") as run, patch.object(browser, "first_reachable", return_value=None), patch.object(browser, "open_browser") as open_tab, patch("sys.stderr", new_callable=io.StringIO):
    ...
    def test_mac_opener_receives_fragment_without_shell(self):
        with patch.object(browser.sys, "platform", "darwin"), patch.object(browser.shutil, "which", return_value="/usr/bin/open"), patch.object(browser.subprocess, "run") as run:
